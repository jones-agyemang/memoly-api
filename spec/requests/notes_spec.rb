...
RSpec.describe "Notes", type: :request do
  ...
  describe "valid bearer token" do
    let(:user) { create(:user, :with_valid_auth_token) }
    ...
    context "when token owner differs from resource owner" do
      let(:authorised_user) { create(:user) }
      let(:authorised_user_access_token) { create(:access_token, user: authorised_user).token }
      let(:unauthorised_user) { create(:user, :with_valid_auth_token) }

      it "does not permit creation" do
        post "/users/#{unauthorised_user.id}/notes", params: valid_attributes, headers: { "Accept": "application/json", "Authorization": "Bearer #{authorised_user_access_token}" }
