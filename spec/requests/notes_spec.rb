...
RSpec.describe "Notes", type: :request do
    ...
    context "with a valid bearer token" do
      context "when collection is undefined" do
        it "creates and assigns a new note in the default collection" do
          travel_to(Time.parse("2025-03-28 15:30:00")) do
            post "/users/#{user.id}/notes", params: valid_attributes, headers: headers
      ...
      context "when params are invalid" do
        it "does not create a note" do
          post "/users/#{user.id}/notes", params: {}, headers: headers

          expect(response).to have_http_status(:bad_request)
        end
      end

      context "when token owner differs from path user" do
        let(:path_user) { create(:user) }

        it "does not permit creation" do
          post "/users/#{path_user.id}/notes", params: valid_attributes, headers: headers
  ...
  describe "PATCH /users/:user_id/notes/:id" do
    let!(:note) { create(:note, collection: create(:collection, user: user), raw_content: "Original note") }
    ...
    context "when a new collection is provided" do
      let!(:new_collection) { create(:collection, user: user, label: "Target Collection") }
    ...
    it "returns bad request when params are missing" do
      patch "/users/#{user.id}/notes/#{note.id}", headers: headers
  ...
  describe "DELETE /users/:user_id/notes/:id" do
    let!(:note) { create(:note, collection: create(:collection, user: user)) }

    it "removes the note for the user" do
      delete "/users/#{user.id}/notes/#{note.id}", headers: headers
    ...
    it "removes associated reminders" do
      reminder = create(:reminder, note: note)
    ...
    it "returns not found when the note belongs to another user" do
      ...
      delete "/users/#{user.id}/notes/#{other_note.id}", headers: headers
