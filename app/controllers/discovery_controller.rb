class DiscoveryController < ApplicationController
  ...
  def collection_matches?(collection, children_map)
    direct_match = collection_label_matches?(collection) || notes_match?(collection.notes)
  ...
  def filtered_notes_for(collection)
    ...
    return notes if @normalized_query.blank? || collection_label_matches?(collection)
  ...
  def collection_label_matches?(collection)
    collection.label.to_s.downcase.include?(@normalized_query)
  end
